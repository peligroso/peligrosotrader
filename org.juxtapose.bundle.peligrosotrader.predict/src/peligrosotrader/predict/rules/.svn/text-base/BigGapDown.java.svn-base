...
public class BigGapDown extends Rule{

	public static int ID = 113;
	static String NAME = "Big gap down"; 
	public static String DESCRIPTION = "Signals when open far under yesterdays close with the parameters set from the intraday movement.";
	...
	/**PARAMS**/
	static int FAINTH_MOVE = 11301;
	static int UP_MOVE = 11302;
	static int DOWN_MOVE = 11303;
	...
	public int signal(Vector<Sample> inGraph) {
		if(m_first){
			...
			double closeOpenDiff = (beforeLastSamp.close / lastSamp.open) - 1;
			...
			if(closeOpenDiff > avgMove * FACTOR){
				...
				System.out.println("close open diff = "+closeOpenDiff+", avg move = "+avgMove);
				if(fainthMove)
					return FAINTH_MOVE;
				else if(goingUp)
					return UP_MOVE;
