...
public class RuleTerm_RSIAdaptNeg extends MiningRule{

	public static double[] LEVELS = new double[]{60};
	...
	class LevelPrediction
	{
		...
		{		
			if(m_positionized){
				...
				if(isUpbow == null)
				{
					isUpbow = inRsi > m_level;
					return false;
				}

				if(isUpbow == false && inRsi > m_level)
				{
					isUpbow = true;
					m_positionized = true;
					...
					double koff = 0;
					if(m_pos != 0){
						double diff = m_negs - m_pos;
						koff = diff / (double)m_negs;
					}
