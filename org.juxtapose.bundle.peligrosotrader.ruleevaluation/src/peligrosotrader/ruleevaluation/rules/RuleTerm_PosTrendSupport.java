...
public class RuleTerm_PosTrendSupport implements RuleTerm {
	
	static int HINDSIGHT = 250;
	static int HOLD_DAYS = 21;
	...
	public double takePos(Vector<Sample> inGraph) {
		...
		if(supportedByPosTrend(lastSample, trendStack, inGraph.size()-1)){
	 ...
	 * @return
	 */
	private boolean supportedByPosTrend(Sample inSamp, TrendStack inTS, int sampIndex){
		Vector<TrendLine> lines = inTS.getTrends();
		
		for(TrendLine tl : lines){
			double trendValueOnIndex = tl.getValueOnIndex(sampIndex);
			if(compare(inSamp.close, trendValueOnIndex) && tl.isPositive() && tl.m_mergeCount > 4)
				return true;
