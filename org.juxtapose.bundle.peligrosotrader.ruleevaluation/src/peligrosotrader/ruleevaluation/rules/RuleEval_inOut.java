...
public class RuleEval_inOut implements RuleType{
	...
	private void evaluateGraph(Vector<Sample> inGraph){
		...
		for(Sample samp : inGraph){
			...
			if(m_posisionized){
				holding++;
				double pos = m_ruleTerm.leavePos(tryVec);
				if(pos != -1){
					System.out.println("exit trade at for ticker "+samp.ticker+" at: "+pos+"  on "+samp.date+"  holding was: "+holding);
					m_exit = pos;
					calcResult();
					m_posisionized = false;					
				}
				...
				double pos = m_ruleTerm.takePos(tryVec);
				if(pos != -1){
					holding = 0;
					System.out.println("enter trade at for ticker "+samp.ticker+" at: "+pos+"  on "+samp.date);
					m_enter = pos;
					m_posisionized = true;
					m_freq.track();
				}
