...
public class RuleTerm_StrongResistanceScalp implements RuleTerm {

	public static int MIN_LENGT_SIZE = 20;
	public static double MIN_STRENGTH = 1;
	...
	public double takePos(Vector<Sample> inGraph) {
		...
		for(HotPoint hp : stack.m_hotPoints)
		{
			...
			if(hasBeenBelow(hp.m_samp, inGraph) && !hp.m_isBrooken && hp.m_strength > MIN_STRENGTH){
				if(hp.m_samp.close <= last.highest){
					
					System.out.println("daily high: "+inGraph.lastElement().highest+" has triggered point: "+hp.m_samp.close);
					
					return hp.m_samp.close;
				}
			}
