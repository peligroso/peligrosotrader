 ...
 *                   Eduardo Ramalho;
 *
 * $Id: XYPlot.java,v 1.44.2.11 2006/10/23 15:25:45 mungady Exp $
 *
            ...
            if (seriesOrder == SeriesRenderingOrder.REVERSE) {
                ...
                for (int pass = 0; pass < passCount; pass++) {
                    ...
                    for (int series = seriesCount-1; series >= 0 ; series--) {
                        ...
                        int polyLineX[] = new int[itemCount];
                        int polyLineY[] = new int[itemCount];
                        int polyIndex = 0;
                        ...
                        for (int item = 0; item < itemCount; item++) {
                            ...
                            double xx = xAxis.valueToJava2D(x, dataArea, domainEdge);

                            RectangleEdge edge = getRangeAxisEdge();
                            double yyHigh = yAxis.valueToJava2D(yHigh, dataArea, edge);
                            double yyLow = yAxis.valueToJava2D(yLow, dataArea, edge);
                            double yyOpen = yAxis.valueToJava2D(yOpen, dataArea, edge);
                            double yyClose = yAxis.valueToJava2D(yClose, dataArea, edge);
                            ...
                            Paint p = renderer.getItemPaint(series, item);
                            Stroke s = renderer.getItemStroke(series, item);
                            ...
                            g2.fill(new Rectangle2D.Double(xx, yyClose, 5, 5));
                            g2.fill(new Line2D.Double(xx, yyClose, 5, 5));
//                            
                            g2.drawPolyline(polyLineX, polyLineY, itemCount);
                            
                            polyLineX[polyIndex] = new Double(xx).intValue();
                            polyLineY[polyIndex] = new Double(yyClose).intValue();
                            polyIndex++;
                        ...
                        Paint p = renderer.getItemPaint(series, 0);
                        Stroke s = renderer.getItemStroke(series, 1);
                        
                        g2.setPaint(Color.black);
                        
//                        g2.draw(new Line2D.Double(50, 50, 50, 700));
                        g2.fill(new Rectangle2D.Double(0, 0, 500, 500));
                        g2.fillPolygon(polyLineX, polyLineY, itemCount);
                    }
                }
            }
            else {
